...
查看命令的帮助（比如find命令）：find /?       # linux跟参数是 -c  ,windows是 /c

---

- 删除一些需要管理员权限的程序：
  rd  /s   文件/文件夹名          （在文件路径框里输入cmd回车，可快速运行）
...
- 查看目录结构：tree     # 默认就是只要文件夹，要是也要看文件，那就 tree /f

- 过滤文件：==pip list | find "wx"==    # 就是找这里面包含wx字符串的

- 修改host后刷新DNS：ipconfig /flushdns

- bat文本中相关：（都是单行）
  - 增加环境变量：set PATH=D:\Qt\Qt5.9.8\5.9.8\msvc2017_64\bin;%PATH%
  
  - 移动路径：cd /D D:\Qt\Qt5.9.8\5.9.8\msvc2017_64
  
  - 初始化另一个脚本中的环境变量：@call "D:\Program Files (x86)\Microsoft Visual Studio\2017\Community\VC\Auxiliary\Build\vcvars64.bat"
  ...
  - 注释：
    ...
    第二种单行注释   :: 单行注释(英文冒号)
    例：（下面就是3行注释）
    :: 1. 加载index_template.html和dir_template.html
    :: 2. 遍历目录，每个目录根据dir_template.html生成dir.html
    :: 3. 根据index_template.html和每个目录dir.html路径生成index.html
    
    
    多行注释，利用goto，后面的标记随意，这里的例子就是 comment
    goto :comment
