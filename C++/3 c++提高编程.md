...
C++11还提供了emplace和emplace_back方法，分别对应insert和push_back方法。

==emplace_back==和==push_back==的区别：

- push_back需要一个已经构造好的对象(可以是临时对象)；
- emplace_back增加了一个：可通过传递构造函数的参数直接在容器中构造对象。它通常比push_back更高效，因为它可以避免不必要的对象拷贝或移动操作。
...
int main() {
    // 先说明一下，要创建一个线程是这样：直接给函数名，后面把参数加进去就好了
    std::thread t1(writer, i, 5);   
    
	std::vector<std::thread> threads;
	for (int i = 0; i < 5; ++i) {
        // 1、emplace_back可以直接接受其所属容器类型的多值构造参数，然后在容器内直接构造
		threads.emplace_back(writer, i, 5); 
        
        // 2、push_back则必须这样先构建一个临时对象传递进去（当然，emplace_back也可以这样传递）
		// threads.push_back(std::thread(reader, i, 5));
	}
...
### 3.7. ==algorithm包含的所有功能== 

以下是 `<algorithm>` 头文件中所有函数的列表：

> - `std::all_of`：用于判断一个范围内的所有元素是否都满足某个条件。
> - `std::any_of`：用于判断一个范围内是否存在任意一个元素满足某个条件。
> - `std::none_of`：用于判断一个范围内是否不存在任何一个元素满足某个条件。
> - `std::for_each`：用于对一个范围内的所有元素执行某个操作。
> - `std::count`：用于计算一个范围内某个值的出现次数。
>
>   - `std::count_if`：用于计算一个范围内满足某个条件的元素的个数。
> - `std::mismatch`：用于查找两个范围中第一个不匹配的元素。
> - `std::find`：用于查找一个值在一个范围内的第一个出现位置。
>   - `std::find_if`：用于查找一个范围内满足某个条件的第一个元素。
>   - `std::find_if_not`：用于查找一个范围内第一个不满足某个条件的元素。
> - `std::adjacent_find`：用于查找一个范围内第一对相邻的相等元素。
> - `std::search`：用于在一个范围内查找另一个范围的第一次出现位置。
>
>   - `std::search_n`：用于在一个范围内查找连续出现 n 次的某个值的第一次出现位置。
> - `std::copy`：用于将一个范围内的元素复制到另一个范围中。
>   - `std::copy_if`：用于将一个范围内满足某个条件的元素复制到另一个范围中。 （用这来去空格）
>
>   - `std::copy_n`：用于将一个范围内的前 n 个元素复制到另一个范围中。
>
>   - `std::copy_backward`：用于将一个范围内的元素以相反的顺序复制到另一个范围中。
>
>     ```
>     std::string a = "  hello   world   shi  123     ";  
>     std::string res;     // 因为res没初始化，用 std::back_inserter，前面提到过
>     // 关于std::isspace，还有更多的其它的关于单个字符的判断，可以去看C++小知识
>     std::copy_if(a.begin(), a.end(), std::back_inserter(res), [](const char c) { return !std::isspace(c); });
>     std::cout << res << std::endl;   // helloworldshi123  这就是对的
>     // std::isspace  std::isblank 都是可以的，用 isspace 吧，还能代表制表符
>     ```
>
>
> - `std::move`：用于将一个范围内的元素移动到另一个范围中。
>
>   - `std::move_backward`：用于将一个范围内的元素以相反的顺序移动到另一个范围中。
> - `std::fill`：用于将一个范围内的所有元素设置为指定的值。
>
>   - `std::fill_n`：用于将一个范围内的前 n 个元素设置为指定的值。
> - `std::transform`：用于对一个范围内的所有元素执行某个操作，并将结果存储到另一个范围中。
> - `std::generate`：用于生成一个范围内的所有元素。
>
>   - `std::generate_n`：用于生成一个范围内的前 n 个元素。
> - `std::remove`：用于从一个范围中删除指定的值。
>   - `std::remove_if`：用于从一个范围中删除满足某个条件的元素。（尽量别用这个，用std::copy_if）
>
>   - `std::remove_copy`：用于将一个范围中不等于指定值的元素复制到另一个范围中。
>
>   - `std::remove_copy_if`：用于将一个范围中不满足某个条件的元素复制到另一个范围中。
>
>     ```
>     std::string a = "  hello   world   shi  123     ";
>     std::remove_if(a.begin(), a.end(), [](const char c) { return std::isblank(c); });
>     // 这个代码是去掉所有空格，但后面的空格没去掉，
>     std::cout << a << std::endl;  // helloworldshi123  shi  123 
>     ```
> - `std::replace`：用于将一个范围中的指定值替换为另一个值。
>   - `std::replace_if`：用于将一个范围中满足某个条件的元素替换为另一个值。
>   - `std::replace_copy`：用于将一个范围中的指定值替换为另一个值，并将结果复制到一个新容器，不改变原来容器的数据。
> - `std::replace_copy_if`：用于将一个范围中满足某个条件的元素替换为另一个值，并将结果复制到另一个范围中。
> - `std::unique`：用于从一个范围中删除相邻的重复元素。
>
>   - `std::unique_copy`：用于将一个范围中相邻的重复元素去重，并将结果复制到另一个范围中。
> - `std::reverse`：用于将一个范围中的元素以相反的顺序排列。
>
>   - `std::reverse_copy`：用于将一个范围中的元素以相反的顺序复制到另一个范围中。
> - `std::rotate`：用于将一个范围中的元素循环移动到指定位置。 
>
>   - `std::rotate_copy`：用于将一个范围中的元素循环移动到指定位置，并将结果复制到另一个范围中。
> - `std::sort`：用于对一个范围中的元素进行排序。
>   - `std::stable_sort`：用于对一个范围中的元素进行稳定排序。
>   - `std::partial_sort`：用于对一个范围中的前 k 个元素进行排序。
>   - `std::partial_sort_copy`：用于将一个范围中的前 k 个元素排序，并将结果复制到另一个范围中。
>   - `std::nth_element`：用于对一个范围中的元素进行部分排序，使第 n 个元素处于正确的位置。
> - `std::merge`：用于将两个已排序的范围合并为一个已排序的范围。
>
>   - `std::inplace_merge`：用于将一个范围中的两个已排序子范围合并为一个已排序的范围。
> - `std::includes`：用于判断一个范围是否包含另一个范围。
> - `std::set_union`：用于将两个已排序的范围合并为一个已排序的范围，并去除重复元素。
>   - `std::set_intersection`：用于计算两个已排序的范围的交集。
>   - `std::set_difference`：用于计算两个已排序的范围的差集。
>   - `std::set_symmetric_difference`：用于计算两个已排序的范围的对称差集。
> - `std::min_element`：用于查找一个范围中的最小元素。
> - `std::max_element`：用于查找一个范围中的最大元素。
> - `std::minmax_element`：用于同时查找一个范围中的最小和最大元素。
> - `std::lexicographical_compare`：用于比较两个范围的字典序大小。
> - `std::next_permutation`：用于生成一个范围中的下一个排列。
> - `std::prev_permutation`：用于生成一个范围中的上一个排列。
...
void test01() {
    ...
    // 方法4：lambda函数(lambda中的参数类型是容器的类型，若是此类型是一个结构体，还可以用结构体中某一个数据类型来进行对比)
...
容器中的std::move就是移动容器内的数据，

- 下面这两种方式不好说，可能有错误，如果是stl容器相关就用方式一；如果是传统数组那种，虽然也可以用指针，但还是用方式二吧
...
// 方式一(跟那些用容器迭代器一个用法)
std::move(a.begin(), a.end(), dest.begin());
// 方式二：
dest = std::move(a);
```
...
- 或者：
  ...
  std::vector<std::string> results{"hello", "world", "this", "is"};
  std::stringstream ss;
