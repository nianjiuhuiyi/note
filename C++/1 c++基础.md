...
`unsigned const  int hread_count = std::thread::hardware_concurrency();`  // 获取当前机器的逻辑核心数。
...
1. ==.join()与.detach()的区别==:
   - join():
     - 当一个线程调用join方法时，它会阻塞当前线程，直到此子线程执行完成;
     - join()返回时，会自动清理回收资源，一定要有。
   - deatch():
        - 当调用detach时，线程将于其所属线程分离，并在后台独立运行；
     - 一旦线程被分离，它将不再受主线程的控制，主线程也不再需要调用join来等待它的结束；
     - 分离的线程会自动回收其资源，当线程执行完毕后，其资源会被系统回收。
     - 建议：写一个类，类中有一个私有变量来控制状态，然后一个成员函数来专门采集视频帧放队列里，这个成员函数通过这个私有变量来判断是否退出while循环，然后把这个成员函数当做多线程启动时，就可以选择 .detach 启动，其它形成中也不再需要 .join
       (OpenCV_C++版本多线程读摄像头我里面就用了这个，在算能盒子的yolov5的cpp代码中， int VideoDecFFM::openDec 成员函数中也用到了，里面启动了一个新线程，std::thread push(&VideoDecFFM::vidPushImage, this); push.detach();)
2. 注意：
   - 当引用传递或是指针传递时，一定要确保对这些数据的访问时线程安全的，否则可能呼吁道数据竞争或是其它并发问题。
   - 新线程中抛出的异常不会自动传播到创建该线程的线程，所以要尽可能确保在新线程中处理好异常。
   - ==创建线程那一刻，它就在执行了==(一定谨记)，不是Python那种还要.start()一下。
   - 线程一定要有.join(）或.deatch(),它们可以确保线程资源正确的回收和清理，因为当join返回时，线程对象所代表的线程已经完成了执行。
3. std::this_thread： 是C++标准库的一个命名空间，用来获取当前线程的特定属性，如线程ID、休眠当前线程等。
   - std::this_thread::get_id();
   - std::this_thread::sleep_for(std::chrono::seconds(2));
   - std::this_thread::sleep_until 到一个具体的时间节点;
...
2. 原子性：对互斥锁的获取和释放操作是原子的，这些操作不会被其它线程打断。

##### (1) std::mutex

基本的互斥锁有四种，它们都是class类：在头文件 `#include <mutex>`中：

- std::mutex                   # 最基本的
- std::recursive_mutex          # 递归锁，与std::mutex不同的事，它允许同一线程多次锁定同一个互斥量。
- std::timed_mutex              # 带时限的互斥量。处理提供加锁解锁操作，它还允许线程尝试在一定时间内锁定互斥量。
- std::recursive_timed_mutex     # 前面的集合体，递归且带时限的互斥量。
...
2. ==std::recursive_mutex==  # 加锁与解锁的次数一定要对应起来，否则也会报错
...
3. ==std::timed_mutex== 
   ...
   - 如果在指定的时间内护法获取锁，.try_lock_for()或.try_lock_until()函数将会返回失败，而线程不会被阻塞，返回成功就直接锁上。
   - 这对实现有超时机制的资源访问非常有用。
...
使用RAII管理锁：
	使用RAII(资源获取即初始化)原则来管理锁的生命周期，通过std::lock_guard或std::unique_lock来确保锁在不用时自动释放。(有点像智能指针的味道)

为什么需要std::lock_guard?

- 单纯使用mutex的时候，容易忘记解锁:如果忘记在某个路径上调用mtx.unlock()，会导致mutex一直保持锁定状态，其他线程将无法继续执行。
- 异常安全:在C++中，如果在持有mutex锁的情况下抛出异常，必须确保在异常发生时mutex能够被正确解锁。手动管理异常路径上的解锁容易出错。
  自动解锁: std::lock_guard在其析构函数中自动调用unlock()，因此即使发生异常，mutex也能被正确解锁。
- 应对太过复杂的逻辑:代码逻辑如果太复杂，你往往不知道在哪里解锁合适。
- 简单易用: std::lock_guard 使用简单，只需在需要锁定的代码块内创建一个std::lock_guard对象即可，无需手动调用lock()和unlock()。
...
1. 锁的粒度比较大：锁的作用域与对象的作用域相同，可能会影响并发性能；
2. 无法中途解锁：一旦std::lock_guard对象构造，锁将自动获取，并在对象析构时才自动释放，无法中途解锁；
3. 不支持条件变量：std::lock_guard通常与互斥锁std::mutex一起使用，但不支持与条件变量(如std::condition_varilable)配合使用；
4. 不止递归锁：虽然可以与std::recursive_mutex一起使用，但std::lock_guard本身不提供对递归锁的特殊支持；
...
1. 灵活性: std::unique_lock提供了更多的控制选项，比如延迟锁定、尝试锁定、递归锁定、定时锁定等。这使得开发者可以根据具体的业务逻辑来精确控制锁的加锁和解锁时机。
2. 可移动性: std::unique_lock是可移动的，可以拷贝、赋值或移动。这使得开发者可以在需要时轻松地将锁的所有权从一个对象转移到另一个对象。
3. 手动控制: std::unique_lock支持手动解锁，而std::lock_guard不支持。这提供了更大的灵活性，但也需要开发者自行管理锁的加锁和解锁过程。

它常用的成员函数：

- .lock(): 锁定关联的mutex;  # ==当这个锁被别的线程持有时，它会阻塞在这里== 
- .unlock(): 解锁关联的mutex;
- try_lock(): 尝试锁定mutex，如果锁定成功返回true，否则返回false;
- owns_loc(): 返回一个布尔值，表示std::unique_lock是否拥有mutex的所有权。
...
1. 基于(时间点)/(时间段)的超时锁定：  （其实时间点、时间段都差不多）
...
1. 释放锁：当线程调用wait方法时，它会首先释放与std::unique_lock关联的互斥锁。这是为了允许其他线程可以访问和修改与条件变量相关的共享数据。
2. 进入等待队列：释放锁后，线程会进入条件变量的等待队列中，进入阻塞状态。此时线程不再消耗CPU时间，直到被唤醒。
3. 等待条件and通知：线程在等待队列中等待，直到下面两个条件都满足：
   ...
   - 条件二：谓词函数 pred 返回true。或是共享变量满足(即while检查)
     ...
     - 如果cdv.wait()所在线程开始执行时，它的共享变量是true或是谓词函数 pred 返回true，哪怕没有任何线程调用notify_通知函数，它也不会被阻塞，会继续向下执行；
       但其它线程一定还是要调用通知函数，不然一旦这个条件有一次变为false，cdv.wait()所在线程开始等待了，哪怕过程中条件再变成了true，如果没有其它线程调用唤醒函数，就会一直卡在这里。
     
     再由下面线程池中的终结出来：（非所有情况，主要还是看共享变量时什么样的）
     
     - 尽量还是用while(共享变量) + 条件变量.wait()仅接收锁一个参数的形式：特别在最后的时候，在while()不满足时，就进不到里面，也不会有等待发生，然而.wait()接收两个参数那种，那第二个参数最后永远返回false，就卡在那里了。
       看线程池“initialize()”函数中的具体代码。
4. 重新获取锁：当线程被唤醒后（无论是由于收到通知还是条件成立），它会尝试重新获取之前释放的互斥锁。如果此时锁已经被其他线程持有，则该线程会阻塞在互斥锁上，直到获得锁。
5. 检查条件：获得锁后，线程会再次调用谓词函数 pred 来检查条件是否成立。如果条件不成立（即pred返回false)，则线程会重新进入等待队列，并释放锁，继续等待。这个过程会不断重复，直到条件成立。（也可以是while循环那种写法，由while去检查条件）
6. 继续执行：如果条件成立(即pred返回true)，则线程会退出wait方法，并继续执行后续的代码。

注意事项：

- wait方法必须在已经锁定互斥锁的情况下调用，否则会抛出std::system_error异常。
- 在调用wait方法前，应确保与条件变量相关的共享数据(如这里的flag)已经被正确初始化，并且已经设置了适当的条件。
- 由于wait方法会释放和重新获得锁，因此在使用wait方法时应确保没有其它线程会在等待期间修改与条件变量相关的共享数据，以避免竞态条件。
- 条件变量的成员函数wait和wait_for,如下面代码它都与互斥锁(std::mutex或==std::unique_lock==)一起使用，而且它等待过程中会释放掉cpu资源。(注意==条件变量这里只能是std::unique_lock==)
- cdv.notify_one()、cdv.notify_one()的调用次数是可以多的，即生产者线程调用了10次，消费者才消费1次，就是可以的，即调用通知时，哪怕没有条件变量在等待，也是无所谓的。
...
- 上面代码 “myprint” 是子线程，它会检查共享变量flag，如果不为true，则代表没准备好，然后调用cdv.wait()来进行阻塞，并释放锁，然后等待其它线程调用cdv.notify()或是cdv.notify_all()来唤醒它；
  ==这我之前肯定是在那里while死循环，每隔1s去判断一次flag是不是为true，是就break，这样会空跑cpu调度==，还是用条件变量来好些。
- 理解wait：别的线程调用了==同一个条件变量实例对象==的.notify_all()后，它收到通知就会去检查设定的共享变量flag是不是满足条件了，如果不是，它还将继续阻塞不会往下执行。
- wait有两种使用方式(上面代码写了)：
  - 方式一：while循环去检查共享变量是否成立，这种wait就只接受一个为锁的参数；
  - 方式二：==wait还接收第二个参数(Predicate pred)谓词，即一个可调用对象(如函数、lambda表达式等)，它返回一个布尔值，为true就是满足条件不再等待，为flase就是条件不满足，再继续等待，相当于回到了阻塞的状态，又等待别的线程调用通知函数==。
...
- 版本二：
  ...
  - 返回值就是bool值：==必须要指定时间内同时收到其它线程的notify_all通知和谓词返回值为true，这的返回结果才会是true==，任意一个不成立都是返回false。
    ...
    说明：
      ...
      - 以上代码会打印没超时；  # 这说明==wait_for需要的两个条件是不需要先后顺序，也不需要同时满足，只要在等待时间内发生就好了== 
      - updateFlag先等1秒就通知了，此时wait_for虽然收到了通知，但是flag还不满足，且此刻还未超时，它就会继续等待，updateFlag2秒后再将flag置为true，这是条件都满足了，且未超时，就会打印没超时。
      - 注意一点：==如果将第20行的等待时间从2秒改成10秒，那显然wait_for是等不到flag变为true了，就应该会超时，但是运行很多次，哪怕从2秒改成60秒，打印的依然是没超时==，我不理解，只能说以后如果真的写代码遇到要用这个，一定要注意慎用。
...
- 教程是把13行的`std::unique_lock<std::mutex> lock(mtx);`放在了for循环内，我将其放在for循环之上也是OK的。
- 这个锁比我想的智能，就这行代码我以为放循环之上，可能有问题，没想到完全可以，一开始我自己写是放循环之上用的`std::unique_lock<std::mutex> lock(mtx, std::defer_lock);`，然后在cdv.wait之前手动加锁lock.lock();再在flag = !flag;之后lock.unlock();也是可以的，从而看来这个锁比我想象的智能多了。
- 这里最核心的思想：cdv.wait，在lambda表达式不满足时，它会释放锁，并重新进入阻塞，就完全替代掉了while循环。一定去看最上面的==工作原理==。如果一开始它没拿到锁，它也会阻塞在这，一直去尝试拿锁，直到拿到。
  ==也就是wait中自己已经包含了尝试去拿锁，没拿到就再去尝试，直到拿到；满足条件后就往下走，再智能释放锁；没满足条件，就自动释放掉锁，再进入阻塞队列，直到别的线程将其唤醒==。

#### 10.2.3 读写锁

​	C++中的读写锁（也称为共享锁(std::shared_mutex、std::shared_lock)和独占锁(std::unique_lock)）是一种同步机制，用于控制对共享资源的访问，允许多个线程同时读取资源，但在写入资源时只允许一个线程独占访问。在读多写少的情况下能显著提升性能。

==std::shared_mutex==是C++17引入的一种互斥锁，用于支持多读单写的并发访问模式，它允许多个线程同时拥有共享锁(读锁),但在持有独占锁(写锁)时，其它线程不能再持有任何类型的锁。
==std::shared_lock==是C++17引入的一种锁的管理器，用于管理std::shared_mutex的共享锁，可以自动获取和释放共享锁，类似于std::mutex与std::unique_lock之间的关系

注意：

std::shared_mutex根据成员函数，是有两种功能的：

1. 排他性锁定(即跟std::unique_lock这种一样)：
   - .lock       # 获取独占锁，如果当前有其它线程持有共享锁或是独占锁，则当前线程会被阻塞；
   - .try_lock   # 尝试获取独占锁，如果锁被其它线程持有，则返回false，否则获取锁并返回true；
   - .unlock     # 释放独占锁
2. 共享锁定：
   - .lock_shared;  # ==获取共享锁，如果当前有其它线程持有独占锁，则当前线程会被阻塞==；（因为独占锁一般是写操作拿到的，那写操作的时候也是不允许有读操作的，所以其它线程有独占锁时，那这肯定会阻塞）
   - .try_lock_shared;  # 尝试获取共享锁，如果当前有其它线程持有独占锁，则返回false，否则获取锁并返回true；
   - .unlock_shared;  # 释放共享锁。
...
- 最好用linux做测试，win上没加锁的myconuter结果都是对的，`g++ 01.cpp -lpthread` 用了多线程记得加 -lpthread
...
1. 为什么有内存序问题？原子操作的内存序问题，[视频地址](https://www.bilibili.com/video/BV1bS421X7sf?t=18.4&p=41)。

   1. 编译器优化：编译器在编译代码时，可能会改变代码中操作的顺序，以便生成更高效的机器代码。
   2. 处理器优化：处理器在执行指令时，可能会改变指令的执行顺序，亦提高执行效率。例如，处理器可能会先执行不依赖于前一条指令结果的操作。
   3. 缓存一致性：处理器缓存中的数据可能与主内存中的数据不同步，不同处理器核心对同一内存位置的读取结果可能不同。

2. 所以原子成员函数有一个参数可以来指定原子操作内存序：
   内存顺序定义了多线程环境中操作的可见性和顺序规划，确保线程间的同步。原子操作可以指定不同的内存顺序，以控制操作的可见性和排序。
   这个参数是一个枚举类型(std::memory_order)，如：std::memory_order::memory_order_seq_cst
...
1. 技术信号量(Counting Semaphore)：允许对资源的多次访问(而不是锁住仅一个线程访问)。计数信号量的值可以是任意非负整数，表示可以同时访问资源的线程或进程的数量。
2. 二元信号量(Binary Semaphore)：也称为互斥信号量(Mutex),其值只能是0或1，就类似于互斥锁了。

信号量的操作：（P操作、V操作都是原子操作）

1. P操作(等待，wait)：如果信号量的值大于0，则将其减去1，否则线程会被阻塞，直到信号量的值大于0。
2. V操作(释放，signal)：将信号量的值加1，如果有线程被阻塞在P操作上，则唤醒一个阻塞的线程。
...
- 计数器：栅栏内部维护一个计数器，记录到达栅栏的线程数量，当计数器达到预设值时，所有等待的线程被同时唤醒；
- 可重用性：C++20中的std::barrier是可重用的，线程可以反复使用同一个栅栏对象进行同步。

作用：

- 阶段同步：确保多个线程在某一阶段完成后再进入下一阶段。例如：在并行计算中，各个线程可能需要同步它们的计算结果，然后再继续下一步计算；
- 批处理：多个线程可能需要在某些关键时刻汇聚数据或状态，然后再继续各自的任务。
...
- .wait_for(一个等待时间)、.wait_until(一个时间点);
  - 用前面的.wait_for(一个等待时间)，其返回值是一个枚举的状态值，没超时就.get()去拿，超时就去干别的，等会来拿,可以看线程池中的应用。
  - 如果异步操作尚未启动(例如，使用std::launch::deferred策略，则返回std::future_status_defrred)
- .wait()后面这个基本不用，它会一直阻塞在那里，直到异步操作运行完毕，但不获取结果。

注意：同一个future对象的.get()方法只能被调用一次，如果多次调用，将抛出 std::future_error 的异常。

#### 10.3.1 promise、future
...
- 简单的传递引用：
  注：这是比较简单，直接把线程的.join()放在了主线程调用变量ret之前，这就相当于成了单线程了，更好的应该是用锁+条件变量来通知，同时确保线程安全（下面代码.join()放在16行打印后的话，task函数执行很久的话，那打印的结果肯定是初始值0）
...
- 更好的方式是std::promise、std::future，他们也是线程安全的（==可能日常使用就用这个吧，就应该够了==）
  ...
  - ==上面代码会先执行到 f.get()， 然后在没执行.set_value()函数，它会阻塞在这里==；
  - 当 .set_value() 函数执行完，它会同时通知.get()可以取值了，上面主线程中的代码就会立即拿到值并往后执行，同时子线程也在同时往后执行；
  - 如果是 .set_value() 函数先执行，那另一个线程执行到 .get() 就能直接拿到值，不会阻塞。
  - 注意 .get()只能调用一次。

- 再进阶：上面是子线程赋值，主线程获取，还可以主线程赋值，子线程获取
  用途：创建子线程时，主线程还没算出要给子线程传递的值，就可以用这
  ...
  - 上面的 std::promise、std::future 对象都是把赋值=、拷贝构造函数都 delete了的，所以对象时不可复制的，所以==传参数时要传递引用==或者==传递指针==或者使用==std::move()==，而不能是值传递。特别是参数`std::future<int> &b`,它在里面调用了b.get()，一定记住`一个future对象，只能调用一次.get()` 

- 再进阶：因为上一个程序存在的问题，假设我要用taks创建很多线程，那不是要创建很多个future对象，就很夸张
  就可以用std::future对象f_in的.share()成员函数来创建一个可以复制的对象
...
- ==std::async==  这里面都没出现std::thread了，其实是做了一层封装  # 如果是比较简单的任务，就用std::async的方式二，直接完成异步子线程操作。
  ...
  - 拿到的返回结果，并不是调用的函数task返回的数据类型，而是上一小节讲的 std::future类型，要用过.get()去取值。
  - 开启多线程，==要拿子线程的返回结果，强烈建议使用std::async的第二种方式==，非常简洁明了，比用std::promise、std::future还要清爽。
  - std::async这个是子线程的函数完全执行完，并有return返回才开OK；用10.5.1的可以在子线程函数中调用 .set_value() 方法，那.get()函数就会被唤醒，并拿到值，而不用等子线程运行完。
...
- [OpenCV_C++版.md](../OpenCV/OpenCV_C++版.md)的“3.7. 多线程读相机(线程安全的队列)”，就是一个比较好的例子，我对线程安全的队列做了类似python功能的改写。
- 下面的线程池代码中的线程安全跟上一条的差不多，基本一个意思，就是把像python功能那样的改写的内容删了，留了一些基本的，还把线程安全的队列中的条件变量删了，由线程池的类进行条件变量的通知。

#### 10.4.2 生产、消费者类

[这里](https://www.bilibili.com/video/BV1rF41197FK?t=257.8)把生产者、消费者各写一个函数，写进了一个自定义类中，可以去做参考吧，我平时可能比较难用到，可能都是线程安全的队列比较多，所以这里不花时间了。

### 10.5. 线程池(支持可变参数列表)

下面代码参考的这个[教程](https://www.bilibili.com/video/BV128411U7pU?t=271.1)。

手动通过线程安全的队列，实现线程池代码，注意一下几点：

- 需要C++17标准，因为代码里的“std::invoke_result”；C++11也可以，具体看代码
- 生成的线程一直是保持运行的，有任务推进队列，就去处理，保证了每个线程的复用，而不是一个任务创建一个线程，主要是看initialize中的while (this->m_RuningStatus) {}，非常好，然后很详细的说明就看代码里的注释；
- 然后就是“submitTask”函数，非常秒，它实现了：
  - ==把接受可变参数的函数包装成了无参的形式==；
  - ==同时把有返回值的函数包装成无返回值的函数push进任务队列==；
  - ==通过10.3小节讲的std::packaged_task来获取函数的返回值==（它其实是包裹了一层，在函数中间就把任务推到任务队列，然后后面就用.get()去等待取值）。
- 支持的是可变参数列表，即==传进去的函数的参数列表可以是很多个，也可以没有参数==。
